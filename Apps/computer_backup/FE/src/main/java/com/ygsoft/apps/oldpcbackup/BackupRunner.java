...
class BackupRunner {
    ...
    public void performBackup(BackupProfile backupProfile) throws IOException {
        ...
        if (backupProfile == null) {
            throw new IOException("Cannot backup with null zzProfile");
        }
    ...
    private void backup(File currentItem) throws IOException {
        ...
        if (currentItem == null) {
            throw new IOException("Cannot backup 'null'.");
        }
        else if (currentItem.isDirectory()) {
            logger.info("Running backup for directory : " + currentItem.getName());
            try {
                backupFolder(currentItem);
            }
            catch (IOException e) {
                throw new IOException(String.format(
                        "Cannot backup the file '%s'.",
            ...
            logger.info("Running backup for file      : " + currentItem.getName());
            try {
    ...
    private void backupFolder(File currentDir) throws IOException {
        ...
        if (currentDir == null) {
            throw new IOException("Cannot backup 'null' Dir.");
        }
    ...
    private void backupSingleFile(File currentFile) throws IOException {
        ...
        if (currentFile == null) {
            throw new IOException("Cannot backup 'null' file.");
        }
