...
import com.ygsoft.apps.oldpcbackup.enums.TextsEnum;
...
public class ComputerBackup extends JFrame {
    ...
    private void actionPerformedProfileDelete(String profileName) {
        ...
        // Set the profile's file full path.
        ...
        if (profileFile.exists()) {
            if (profileFile.delete()) {
                Messages.showMessage(
                        ...
                        "The profile had been deleted successfully."
                );
            }
            else {
                Messages.showMessage(
                        Messages.MESSAGE_ERR,
                        "Could not delete the profile's file."
                );
            }
        }
        else {
            Messages.showMessage(
                    Messages.MESSAGE_ERR,
                    "The profile's file does not exist."
            );
        }


        // Refresh the profile names.
        profileNames = getProfileName();
        ...
        // Disable the 'run backup' and 'delete' buttons.
        bProfileDelete.setEnabled(false);
        bProfileBackup.setEnabled(false);
    }
    ...
    private void actionPerformedProfileSelect() {
        ...
        logger.info("Selected profile = " + readProfileName);
        profileSelected = true;


        // Set the profile name in the title.
        this.setTitle(String.format("%s  Active profile : [%s]",
    ...
    private void actionPerformedRunBackup(String profileName) {
        ...
        if (profileSelected) {
            ...
            try {
                ...
                Messages.showMessage(Messages.MESSAGE_ERR, "Cannot parse the profile's file.");
                return;
            }
            ...
            Messages.showMessage(Messages.MESSAGE_INF, "The backup ended successfully.");
        }
        else {
            Messages.showMessage(Messages.MESSAGE_ERR, TextsEnum.T_NO_PROFILE_YET.getText());
        }
    ...
    private void createNewProfile() {
        ...
        bConfirm.addActionListener(e->{
            ...
            if (readItemsToBackup == null || readItemsToBackup.isEmpty()) {
                Messages.showMessage(Messages.MESSAGE_ERR, "No items to backup defined.");
                return;
            }
            ...
            Messages.showMessage(
                    Messages.MESSAGE_INF,
                    String.format ("The profile '%s' created successfully.", readProfileName
            ));


            // Close the new profile's form
            fNewProfile.dispose();


            // Set the new profile to the list.
            ddProfileNames.removeAllItems();
    ...
    private String getItem() {
        ...
        jfc.setDialogTitle("Choose item to backup.");
    ...
    private void createProfile(String theProfileName, String theTargetFolder, String theItems) throws IOException {
        ...
        try {
            ...
            throw new IOException("Cannot create the profile: " + e.getMessage());
        }
    }
